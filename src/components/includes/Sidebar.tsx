...
export class Sidebar extends React.Component<{}, {}> {
    ...
    createNav = () => {
        ...
        const items: itemNav[] = [
            { text: "Home", url: "/homepage", fa_icon: "fab fa-houzz", color: "red-700" },
            { text: "Portfolio", url: "/portfolio", fa_icon: "fas fa-briefcase", color: "blue-200" },
            { text: "Resume", url: "/resume", fa_icon: "fab fa-black-tie", color: "green-600" },
            { text: "Skills", url: "/skills", fa_icon: "fas fa-cogs", color: "pink-500" },
            { text: "About me", url: "/about", fa_icon: "fas fa-address-card", color: "red-600" },
            { text: "Contact", url: "/contact", fa_icon: "fas fa-envelope-open", color: "red-500" },
        ]

        let nav: any = []
        items.map((item, i) => {
            nav.push(
                <li key={i} className="mr-3 flex-1">
                    <NavLink activeClassName={`border-${item.color} text-${item.color} hover:text-${item.color} spanWhite `} to={item.url}
